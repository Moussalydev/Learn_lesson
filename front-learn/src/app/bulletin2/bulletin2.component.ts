...
@Component({
  ...
  styleUrls: ['./bulletin2.component.scss']
})
export class Bulletin2Component implements OnInit {
 ...
 Find_Eleve(matricule){
  ...
  sendToBack(bulletin){
    this.bulletinTwoService.AjouterMoyenne(bulletin)
    .subscribe(data =>
      console.log("success"), 
       error =>
  ...
  MoyenneClasse(classe){
    this.bulletinTwoService.MoyenneDeLaClasse(classe)
      .subscribe(data => {
